...
Experiment Lab — демонстрационный проект для портфолио Product Analyst /
Data Analyst Intern. Он показывает A/B testing как аналитический процесс:
гипотеза → группы → метрики → статистический тест → интерпретация →
ограничения.

Проект ориентирован на рынок РФ/СНГ и написан так, чтобы его можно было быстро
показать на собеседовании: есть synthetic data, PostgreSQL schema, FastAPI,
Streamlit dashboard, расчёт продуктовых метрик и базовая статистическая
интерпретация.
...
deterministic control/treatment assignment
        ↓
conversion rate, ARPU, AOV, purchase rate
        ↓
uplift, p-value, confidence interval
        ↓
FastAPI + Streamlit dashboard
```

Важно: данные в проекте synthetic. Проект не утверждает реальный бизнес-эффект
и не использует реальных пользователей.
...
- Добавить SRM check для контроля перекоса групп.
- Добавить power analysis перед запуском эксперимента.
- Добавить guardrail metrics.
- Добавить CUPED для снижения дисперсии.
- Добавить стратифицированный assignment.
- Добавить экспорт аналитического отчёта по эксперименту.
- Добавить screenshots Swagger и примеров API-запросов.
- Добавить отдельный screenshot database logs / SQL query result.

## Формулировки для резюме

### Product Analyst Intern

- Разработал демонстрационный A/B testing lab для проверки checkout-гипотезы:
  control/treatment split, продуктовые метрики, uplift и статистическая
  интерпретация.
- Реализовал dashboard на Streamlit для объяснения experiment flow: гипотеза,
  группы, conversion rate, ARPU, AOV, p-value и confidence interval.
- Подготовил воспроизводимый demo-сценарий на synthetic event data, который
  показывает путь от событий пользователей до продуктового вывода.

### Data Analyst Intern

- Спроектировал PostgreSQL schema для users, events, experiments, assignments и
  experiment results; добавил индексы и ограничения целостности.
- Реализовал расчёт conversion rate, ARPU, AOV, purchase rate и uplift на Python
  с использованием pandas/SciPy/statsmodels.
- Покрыл ключевую логику pytest-тестами: assignment, metrics engine, API,
  demo setup и dashboard helpers.

## Рассказ о проекте для интервью

### 60-секундный рассказ

Experiment Lab — это мой демонстрационный проект по A/B-тестированию для
продуктовой аналитики. Я сделал его, чтобы показать полный цикл работы
аналитика: сформулировать гипотезу, подготовить synthetic event data,
разделить пользователей на control и treatment, посчитать conversion rate,
ARPU, AOV и purchase rate, оценить uplift, p-value и confidence interval, а
затем показать результат в dashboard и через API.

Ключевой demo-эксперимент — `big_data_checkout_test`. Он моделирует изменение
checkout и показывает, как аналитик переходит от событий пользователей к
аккуратному выводу: есть ли эффект, насколько он большой и насколько ему можно
доверять. Я отдельно зафиксировал ограничения: данные synthetic, нет SRM check,
нет power analysis и нет методов вроде CUPED. Поэтому проект не утверждает
реальный бизнес-эффект, а демонстрирует мой подход к A/B testing и упаковке
аналитики.

### 10 вопросов и ответов для интервью

**1. Зачем нужен deterministic assignment?**  
Чтобы один и тот же пользователь при повторном расчёте попадал в ту же группу.
Так результат анализа становится воспроизводимым.

**2. Почему assignment хранится отдельно от events?**  
Events описывают поведение, а assignments — участие в эксперименте. Разделение
позволяет соединять exposure и поведение без смешивания сущностей.

**3. Почему primary metric — conversion rate?**  
Checkout-изменение должно влиять на завершение покупки, поэтому доля
пользователей с purchase напрямую связана с гипотезой.

**4. Зачем нужны ARPU и AOV, если есть conversion rate?**  
Conversion может вырасти, но выручка или средний чек могут ухудшиться.
Дополнительные метрики помогают не принять однобокое решение.

**5. Как ты объяснишь uplift?**  
Uplift — это разница между treatment и control. Absolute uplift показывает
разницу в единицах метрики, relative uplift — относительное изменение.

**6. Как ты объяснишь p-value?**  
P-value показывает, насколько наблюдаемая разница совместима с нулевой
гипотезой. Это не вероятность победы treatment.

**7. Что показывает confidence interval?**  
Он показывает неопределённость оценки эффекта. Если интервал пересекает 0,
направление эффекта нельзя считать устойчивым.

**8. Почему для conversion rate выбран z-test?**  
Conversion rate — бинарная метрика на уровне пользователя. Для сравнения двух
долей в базовой версии подходит two-proportion z-test.

**9. Почему для ARPU/AOV используется Welch t-test?**  
Это простой способ сравнить средние между двумя независимыми группами без
предположения о равенстве дисперсий. Для demo-проекта он понятен и проверяем.

**10. Какие главные ограничения проекта?**  
Данные synthetic, нет SRM check, нет power analysis, нет CUPED, нет sequential
testing и нет проверки пересекающихся экспериментов. Поэтому выводы нужно
читать как демонстрацию аналитического процесса, а не как решение для реального
продукта.
